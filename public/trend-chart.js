...
export function mountTrendChart(host, range) {
  ...
  function draw(){
    ...
    for(let i=0;i<values.length;i++){if(i%labelStep!==0&&i!==values.length-1)continue;if(i!==values.length-1&&i>values.length-1-labelStep*.65)continue;const x=points[i][0];grid+=`<line class="trend-grid vertical" x1="${x}" y1="${top}" x2="${x}" y2="${bottom+26}"/><text class="trend-date" x="${x}" y="${H-10}" text-anchor="${i===0?'start':i===values.length-1?'end':'middle'}">${date(i)}</text>`;}
