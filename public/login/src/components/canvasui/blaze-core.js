// Generated from the installed @canvas-ui/blaze-react source.
// Run npm run build:blaze after updating the registry component.
const DEFAULTS = {
    height: 0.97,
    distortion: 0.6,
    distortionScale: 0.5,
...
export function createBlaze(elements, options = {}) {
    const config = {
        ...DEFAULTS,
        ...options
    };
    ...
    function ensureFireTarget() {
        ...
        gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, fireTexture, 0);
    ...
    function render() {
        ...
        gl.uniform1i(firePass.uniforms.uLayers, Math.min(Math.max(Math.round(config.layers), 1), 10));
        gl.uniform1f(firePass.uniforms.uSmoke, config.smoke);
        gl.uniform1f(firePass.uniforms.uGlow, config.glow);
        gl.uniform3f(firePass.uniforms.uSparkColor, config.sparkColor[0], config.sparkColor[1], config.sparkColor[2]);
        gl.uniform3f(firePass.uniforms.uSmokeColor, config.smokeColor[0], config.smokeColor[1], config.smokeColor[2]);
        gl.bindFramebuffer(gl.FRAMEBUFFER, fireFbo);
        ...
        gl.uniform1f(mainPass.uniforms.uDistortionScale, Math.max(config.distortionScale, 0.05));
