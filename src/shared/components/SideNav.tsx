...
function ItemGlyph({ item, size = SIDE_NAV_DEFAULTS.menuIconSize }: { item: SideNavItem; size?: number }) {
  if (item.Icon) return <span aria-hidden="true" data-nav-glyph={item.key} className="inline-flex shrink-0"><item.Icon size={size} className="shrink-0 text-[var(--sidenav-icon,#252632)]" /></span>
...
export default function SideNav({
  ...
  return (
    <aside
      ...
      } ${widthClass} ${chromeClass} ${topPaddingClass} ${surfaceClassName}`}
    >
      <motion.div
        initial={false}
