import CreatorNavGlyph from './CreatorNavGlyph'
...
const animatedNavIcon = (name: string) => function NavIcon({ size }: { size?: number | string; className?: string }) {
  return createElement(CreatorNavGlyph, { name, size })
}

/** 左侧栏菜单（数据高亮为当前页），图标统一来自 MasterIcon。 */
export const SIDE_MENU = [
  { key: 'data', label: '首页', Icon: animatedNavIcon('data') },
  { key: 'content', label: '内容管理', Icon: animatedNavIcon('content') },
  { key: 'datacenter', label: '数据中心', Icon: animatedNavIcon('datacenter') },
  { key: 'income', label: '收入变现', Icon: animatedNavIcon('income') },
  // 创作服务 = 一组创作便捷工具（不是客服），用闪电（统一导航稿 lightning-02）
  { key: 'service', label: '创作服务', Icon: animatedNavIcon('service'), children: ['作品共创', '活动管理', '原创保护', '抖音指数'] },
]
