...
function SideNav({ active, onSelect }: { active: string; onSelect: (key: string) => void }) {
  ...
  return (
    <SharedSideNav
      ...
      showDivider={false}
      surfaceClassName="creator-home-side-nav"
      style={{
...
export default function CreatorCenterHome({
  ...
  const page = 'data'
  const setPage = (next: string) => { if (next !== 'data') toast('当前为独立首页演示') }
    ...
    <div className={`flex h-full min-h-0 ${navVersion === 1 ? 'bg-transparent' : 'bg-[#F5F6F8]'}`}>
      ...
      >
        ...
        >
          ...
          <div
            aria-hidden="true"
            className={`pointer-events-none absolute inset-x-0 top-[var(--cc-top)] h-[112px] bg-gradient-to-b from-white via-white/72 to-transparent transition-opacity duration-200 ${
